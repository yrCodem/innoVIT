...
const Header = () => {
  ...
  return (
    <div className="header flex justify-between items-center max-w-[100vw] h-[12vh] bg-transparent font-sora font-black fixed z-50 w-full">
      ...
      <div
        className="md:hidden absolute right-8 z-60 cursor-pointer focus:outline-none focus:ring-0"
        onClick={toggleMenu}
        tabIndex={0}
        onKeyDown={(e) => {
      ...
      <div className="header-right hidden md:block">
        <ul className="list-none flex">
          ...
          >
            <Link
              to="/"
              className="flex h-10 items-center text-sm hover:text-white transition-colors focus:outline-none focus:ring-0"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="20px"
          ...
          <li className="mr-8">
            <Link
              ...
              className="group p-3 border-[1.5px] border-white/30 rounded-full text-textColor bg-white/10 backdrop-blur-md hover:bg-white hover:text-primary flex items-center transition-all duration-300 w-full h-full hover:border-white focus:outline-none focus:ring-0"
              id="header-links"
            >
              <svg
                className="text-textColor group-hover:text-primary transition-colors"
                xmlns="http://www.w3.org/2000/svg"
                height="20px"
                viewBox="0 -960 960 960"
                width="20px"
      ...
      {/* Mobile Menu Overlay */}
      {isMenuOpen && (
        <div className="md:hidden fixed top-[12vh] left-0 right-0 bottom-0 bg-black/80 backdrop-blur-md z-50">
          <div className="bg-primary/90 backdrop-blur-md m-4 p-6 rounded-2xl border border-white/20">
            <ul className="text-center space-y-6">
              <li className="text-textColor">
                <Link
                  to="/"
                  className="flex justify-center items-center text-lg hover:text-white transition-colors focus:outline-none focus:ring-0"
                  onClick={toggleMenu}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <li className="text-textColor">
                <Link
                  to={isAuthenticated ? "/profile" : "/login"}
                  className="flex justify-center items-center text-lg group hover:text-white transition-colors focus:outline-none focus:ring-0"
                  onClick={toggleMenu}
                >
                  <svg
                    className="text-textColor group-hover:text-primary transition-colors"
